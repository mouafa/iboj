...
<section class="latest-messages-container">
  <p class="section-title">Profile Views</p>
  <div v-show="!isChartReady" class="loading-text">Loading...</div>
  <div class="weeklyViewsChart chart-container" id="weeklyViewsChart" v-show="isChartReady" transition="appear"></div>

    <p class="section-title">Last Visitors</p>
  <!-- <div class="topJoboffers-container" v-show="isChartReady && topJoboffers.length" transition="appear">
    <p class="sub-section-title center">Top Offers</p>
    <ul class="collection no-border">
      <li class="collection-item fx-row" v-for="item in topJoboffers | limit 2" track-by="$index">
        ...
        <div class="podium-details fx-center">
          <p class="podium-company-name"><a :href="host + '#!/joboffer/' + item.slug" target="_blank">{{item.company.name}}</a></p>
          <p class="podium-joboffer-title">
            {{item.title}}
          </p>
        </div>
