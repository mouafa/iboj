...
<template>
    <div v-show="isReady">
        <div v-if="jobList.length" class="hpanel p-none">
            <ul class="">
             <li v-for="item in jobList" class="collection-item p-xs fx-col">
               ...
               <div class="fx-row fx-end-start"><button @click="duplicate(item.id)" class="btn-ghost right capital text-orange">duplicate</button></div>
             </li>
           </ul>
        </div>
        ...
        <section v-else class="fx-col fx-start-center placeholder">
           ...
           <p class="m-none capital center">You have no closed job offers</p>
       </section>
    </div>
</template>
<script>
